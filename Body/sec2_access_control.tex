...
%     \begin{definition}
%     \begin{itemize}
%         \item The \alert{PDP (Policy Decision Point~\cite{RFC2753})} is where decisions are made.
%         \item The \alert{PEP (Policy Enforcement Point~\cite{RFC2753})} is where decisions are actually enforced.
%         \item The \alert{PIP (Policy Information Point~\cite{hu2013guide})} delivers information required for the access decision.
%         \item The \alert{PAP (Policy Administration Point~\cite{RFC3198})} creates the access policies.
%     \end{itemize}
%     \end{definition}
% \end{frame}
...
\begin{frame}{Hierarchical architecture: Spacial or Logical separation}
    \begin{center}
        \only<1>{\includegraphics[scale=0.36]{Figures/hierarchical_archi_uni.png}}
        \only<2>{\includegraphics[scale=0.36]{Figures/hierarchical_archi_office.png}}        
        \end{center}
\end{frame}
...
% * * * * * * NEW FRAME * * * * * * %
\begin{frame}{Distributed architecture: Hybrid architecture}
    \begin{center}
...
% * * * * * * NEW FRAME * * * * * * %
\begin{frame}{Distributed architecture: Blockchain-based architecture}
    \begin{center}
...
% \begin{center}
%     \colorbox{lightgray}{\textcolor{orange}{O}: Objective
%  - \textcolor{purple}{C}: Comparative}
% \end{center}
 ...
 \begin{frame}{Comparing architectures: Benefits and disadvantages}
     ...
     {\tiny
          ...
          \multirow{-2}{*}{\textbf{Criteria}} & Serverless & Not serverless & \multirow{-2}{*}{Hierarchical}& \multirow{-2}{*}{Federated}& multi PDP & Hybrid & Blockchain \\
         	...
         	\hline
         %%% META CRITERIA %%%
         Resource Efficiency        & +		& ++ 	& +     & +  	& -  	& - -  &  - -   \\  \arrayrulecolor{black!30}\hline
...
\begin{frame}{Taxonomy}
        \begin{center}
            \only<1>{\includegraphics[scale=0.37]{Figures/taxo.png}}
            \only<2>{\includegraphics[scale=0.37]{Figures/taxo_2.png}}
        \end{center}
\end{frame}
...
% % * * * * * * NEW FRAME * * * * * * %
% \begin{frame}{Litterature review}
%     \begin{table}[ht!]
% \center
% {tiny
%  \begin{tabular}{!{\color{black}\vrule} w w B{1.75cm} W{1.4cm}  b B{1cm} b!{\color{black}\vrule}}
%   \hline
%  \multicolumn{2}{w}{Centralized} &  &  & \multicolumn{3}{b|}{Distributed}\\
%   Serverless & Not serverless & \multirow{-2}{*}{Hierarchical}& \multirow{-2}{*}{Federated}& multi PDP & Hybrid & Blockchain \\
% 	\hline
% 	\hline
% %%% META CRITERIA %%%
% \cite{tamboli2016secure}        & \cite{fernandez2017model} & \cite{barka2015securing}     & \cite{AnggorojatiMahallePrasadEtAl2012}  	& \cite{wu2017implementation}  	& \cite{hemdi2016using}  &  \cite{ZyskindNathanPentland2015}  \\  \arrayrulecolor{black!30}
% \cite{RotondiSecciaPiccione2011}& \cite{bandara2016access}  & \cite{ning2015aggregated}     & \cite{xu2018federated}  	& \cite{fotiou2016access} 	& \cite{bernabe2016taciot}    &  \cite{ouaddah2016fairaccess}  \\  \arrayrulecolor{black!30}
% \cite{HummenShafaghRazaEtAl2014}& \cite{ashibani2017context}& \cite{castiglione2016cryptographic}     & \cite{liu2017access}  	& \cite{patel2016energy} 	& \cite{hussein2017access}    &  \cite{novo2018blockchain}   \\  \arrayrulecolor{black!30}
% \cite{ray2017using}	            & \cite{hao2019fine}		& \cite{hsiao2019elliptic}     & \cite{uriarte2016sensing}  &    &  \cite{cerf2015access} & \cite{pinno2017controlchain}  \\  \arrayrulecolor{black!30}
% \cite{ahmad2018lazy} 	        & \cite{yan2019iot}     	&   &\cite{saadeh2018hierarchical} &     & \cite{SeitzSelanderGehrmann2013}    & \cite{xu2018blendcac}    \\  \arrayrulecolor{black!30}
%  	    &  		&       & \cite{sciancalepore2018design} 	&    	& \cite{Hernandez-RamosJaraMarinEtAl2013} &  \cite{zhang2018smart}  \\  \arrayrulecolor{black!30}\hline
% \end{tabular}
% }
% % \caption{Comparison of architectures}\label{archiCompTable}
% \end{table}
% \end{frame}
